# ggdes/skills/pptx/scripts/office/

## Responsibility

Core Office file manipulation utilities shared by both the PPTX and DOCX skill workflows. Provides the unpack-edit-pack cycle: extract Office XML from ZIP, provide XML-level editing helpers, validate against OOXML XSD schemas, and repack into a valid Office file.
...
- **Invoked by**: Both `pptx/scripts` and `docx/scripts` workflows. `unpack.py` and `pack.py` are the entry points for the XML editing workflow described in `ggdes/skills/pptx/editing.md` and `ggdes/skills/docx/SKILL.md`.
- **Shared code**: These files are identical copies in both `pptx/scripts/office/` and `docx/scripts/office/`. The `office/` package is referenced via `from office.soffice import ...` and `from validators import ...` in the scripts, relying on the package layout within each format's `scripts/` directory.
