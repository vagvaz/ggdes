...
class OutputAgent(ABC):
    ...
    def _generate_and_log_diagrams(self, diagrams_dir: Path) -> None:
        ...
        Checks for pre-existing diagram images in ``diagrams_dir`` first.
        If diagrams already exist (e.g. from a prior format agent in the same
        pipeline run), skips regeneration and reuses them.

        Args:
            diagrams_dir: Directory to write diagram images into.
        """
        # Skip if diagrams were already generated by another agent
        existing = list(diagrams_dir.glob("*.png")) if diagrams_dir.exists() else []
        if existing:
            console.print(
                f"  [dim]Using {len(existing)} existing diagram(s) from "
                f"{diagrams_dir}[/dim]"
            )
            return
